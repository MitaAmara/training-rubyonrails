require 'test_helper'
    class ArticleTest < ActiveSupport::TestCase
        ...
        test "create article with invalid, title is blank" do
            article = Article.new(title: "", content: "lorem ipsum heroda kara",status: "Rilis")
            assert_equal false, article.valid?
            assert_not article.save
        end

        test "create article with invalid data, title < 5" do
            article = Article.new(title: "abcd", content: "lorem ipsum heroda kara",status: "Rilis")
            assert_equal false, article.valid?
            assert_not article.save
        end

        test "create article with invalid data, content is blank" do
            article = Article.new(title: "learn uni test", content: "",status: "Rilis")
            assert_equal false, article.valid?
            assert_not article.save
        end

        test "create article with invalid data, content < 10" do
            article = Article.new(title: "learn uni test", content: "abcde fg",status: "Rilis")
            assert_equal false, article.valid?
            assert_not article.save
        end
        ...
        test "create article with valid data" do
            article = Article.new(:title => "Testing", :content => "This is body", status: 'Rilis')
        ...
        test "edit article with invalid, title is blank" do
            article = Article.first
            assert_equal false, article.update(title: "", content: "lorem ipsum heroda kara",status: "Rilis")
        end

        test "edit article with invalid data, title < 5" do
            article = Article.first
      assert_equal false, article.update(title: "abcd", content: "lorem ipsum heroda kara",status: "Rilis")
        end

        test "edit article with invalid data, content is blank" do
            article = Article.first
            assert_equal false, article.update(title: "learn uni test", content: "",status: "Rilis")
        end

        test "edit article with invalid data, content < 10" do
            article = Article.first
            assert_equal false, article.update(title: "learn uni test", content: "abcde fg",status: "Rilis")
        end
        ...
        #--------------Test custom Scope-----------------
        test "check scope, data exist if status rilis" do
            assert Article.status_rilis
        end
    end